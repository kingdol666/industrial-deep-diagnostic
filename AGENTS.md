...
> **Harness skill mirrors (generated — never edit directly)**: `.agents/skills/` (generic agent discovery, e.g. ZCode) and `.hermes/skills/` (Hermes CLI `skills.external_dirs`) are byte-identical mirrors of `.claude/skills/`, regenerated by `node scripts/sync-harness-skills.mjs` (`--check` = drift guard, exit 1 on drift). After ANY change to `.claude/skills/`, re-run the sync. The retired hand-merged Hermes mega-bundle `.hermes/skills/industrial-deep-diagnostic/` was removed in favor of the mirror (its 14-agent current equivalent lives in `.claude/skills/industrial-analysis-auto`). Agent-definition surfaces other than `.omp/agents/`: `.claude/agents/` (legacy Claude-Code-native set, not used by the pipeline), `.codex/agents/*.toml` (thin loaders that read the skill-side protocol at runtime), `.hermes/agents/*.md` + `agents.yaml` (Hermes `delegate_task` launch stubs).
