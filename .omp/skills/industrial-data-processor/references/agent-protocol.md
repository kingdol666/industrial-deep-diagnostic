...
- [ ] **5.5.1** Determine VLM availability: check `VLM_ENABLED` env/context. If `VLM_ENABLED=true` and vision model available, proceed to dispatch.
...
- [ ] **5.5.3** **Dispatch vlm-visual-analyzer Agent** via task():
  ```javascript
  task({
  ...
  Read each PNG in plot_manifest priority order. Extract structured visual observations.
...
- [ ] **5.5.5** **Anti-forgery verification**: run `node "$SKILL_PATH/scripts/vlm-verification-check.mjs" "$RUN_DIR"`. Verify `analysis_provenance.source_agent == "vlm-visual-analyzer"` and `analysis_provenance.skeleton_overwritten == true`.
...
- Gate: visual_analysis.json exists with valid analysis_provenance. If VLM was used, skeleton was overwritten.
