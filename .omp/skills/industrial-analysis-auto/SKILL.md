...
每个子步骤使用 OMP `task` 工具调度对应的 sub-agent：

```javascript
// Step 2+2.5: Ontology Builder
task({
  agent: "context-builder",
  task: `DATA_PATH=<data-path>
RUN_DIR=<run-dir>
SKILL_PATH=<path-to-.omp/skills/industrial-ontology-builder>
...
task({
  ...
  task: `DATA_PATH=<data-path>
RUN_DIR=<run-dir>
SKILL_PATH=<path-to-.omp/skills/industrial-data-processor>
SHARED_PATH=<path-to-.omp/shared>
...
task({
  ...
  task: `DATA_PATH=<data-path>
RUN_DIR=<run-dir>
SKILL_PATH=<path-to-.omp/skills/industrial-diagnostician>
SHARED_PATH=<path-to-.omp/shared>
...
task({
  agent: "reporter",
  task: `RUN_DIR=<run-dir>
SKILL_PATH=<path-to-.omp/skills/industrial-reporter>
SHARED_PATH=<path-to-.omp/shared>
...
task({
  ...
  task: `RUN_DIR=<run-dir>
SKILL_PATH=<path-to-.omp/skills/industrial-html-visualizer>
SHARED_PATH=<path-to-.omp/shared>
...
task({
  agent: "html-reviewer",
  task: `RUN_DIR=<run-dir>
SKILL_PATH=<path-to-.omp/skills/industrial-html-reviewer>
SHARED_PATH=<path-to-.omp/shared>
...
Read `skill://industrial-ontology-builder` and dispatch via `task({agent: "context-builder", ...})`. Key:
...
Read `skill://industrial-data-processor` and dispatch via `task({agent: "data-processor", ...})`. **ontology_first** — read ontology before any statistical work.

Post-processing after agent completes:
```bash
SKILL_PATH_DATA_PROCESSOR="$PROJECT_ROOT/.omp/skills/industrial-data-processor"
...
Read `skill://industrial-diagnostician` and dispatch via `task({agent: "diagnostician", ...})`. Fuses data + ontology + physics + VLM + time-lag → diagnosis/evidence/confidence/reasoning_chain.
...
Read `skill://industrial-judge` and dispatch via `task({agent: "judge", ...})`. 10-item quality gate → `judge_feedback.json`.

### Step 5b: Physical Auditor (Pre-Report)

Read `skill://industrial-physical-auditor` with `PRE_REPORT_AUDIT=true`. Dispatch via `task({agent: "report-reviewer", ...})`. Runs **parallel** with Judge. Outputs `optimizer_preflight.md`.
...
Read `skill://industrial-reporter` and dispatch via `task({agent: "reporter", ...})`. **Judge-gated**: only proceed if verdict==pass ∧ score≥90 OR `judge_repair_summary` proves 3 rounds exhausted.

**CP-7**: `report.md` + `run_summary.json` exist

### Step 7: Physical Auditor (Final)

Read `skill://industrial-physical-auditor` (final mode) and dispatch via `task({agent: "report-reviewer", ...})`. Audits `report.md` → `optimizer.md`.

**CP-8**: `optimizer.md` contains `ENDORSED`

### Step 8: HTML Visualizer (AUTO)

Read `skill://industrial-html-visualizer` and dispatch via `task({agent: "html-visualizer", ...})`. **Non-interactive** — after CP-8 ENDORSED, immediately launch without asking. Only skip if `00_input/html_opt_out` exists.

### Step 8.5: HTML Reviewer

Read `skill://industrial-html-reviewer` and dispatch via `task({agent: "html-reviewer", ...})`. Review → pass/needs_revision.
...
```bash
SKILL_PATH="<path-to-.omp/skills/industrial-analysis-auto>"
SHARED_PATH="<path-to-.omp/shared>"
...
| 2 | Main agent reads sub-agent protocol then executes itself | Use `task({agent: "...", task: `...`, effort: "hi"})` |
