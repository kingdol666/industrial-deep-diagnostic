...
### Check 7: Root Cause Convergence (CRITICAL)

**The diagnosis MUST converge to a SINGLE most probable root cause, not end with a list of hypotheses.**

1. **Single conclusion check**: Does the diagnosis produce ONE definitive root cause?
   - If the diagnosis ends with "several possible causes: H1 (65%), H2 (55%), H3 (40%)" → **BLOCKING ISSUE**. The Diagnostician has not done its job.
   - The diagnosis must state: "ROOT CAUSE: [specific parameter] → [specific mechanism] → [specific defect]"

2. **Convergence scoring check**: Does the diagnosis include a 3-dimension convergence score (Data 0-35 + Physics 0-35 + Logic 0-30)?
   - If convergence scoring is missing → **WARNING**

3. **Integrated evidence chain check**: Is the root cause supported by ALL THREE dimensions?
   - Data: specific time-matched correlation at optimal lag, with validation status
   - Physics: complete causal mechanism with quantitative magnitude check
   - Logic: [OBSERVED]/[INFERRED] chain, counterfactual elimination, falsifiability
   - If any dimension is missing → **WARNING**
   - If the primary root cause relies primarily on one dimension (e.g., strong r but no physical mechanism) → **BLOCKING ISSUE**

4. **Anti-pattern detection**:
   - "Several possible causes exist..." → **BLOCKING** (diagnosis not converged)
   - "Parameter X may be related to defect Y" → **BLOCKING** (too vague)
   - "More data is needed to determine root cause" → **BLOCKING** unless ALL hypotheses scored < 50 on convergence
   - Ending with ranked hypothesis list without synthesis → **BLOCKING**
