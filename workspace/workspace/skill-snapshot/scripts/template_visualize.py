...
def detect_data_pattern(df, time_col=None):
    ...
    # Classify process vs quality columns
    pq_classification = classify_process_quality_columns(df, numeric_cols, time_col)
    ...
    return {
        ...
        'total_rows': len(df),
        'process_columns': pq_classification['process_cols'],
        'quality_columns': pq_classification['quality_cols'],
        'unknown_columns': pq_classification['unknown_cols'],
        'pq_classification_confidence': pq_classification['confidence'],
    }
...
def classify_process_quality_columns(df, numeric_columns, time_col=None):
    """
    Classify numeric columns as process parameters or quality/inspection metrics.

    Industrial heuristics:
    - Process params: temperature, pressure, speed, flow, voltage, current, power,
      rpm, torque, frequency, level, setpoint, etc.
    - Quality metrics: thickness, defect, weight, dimension, diameter, grade,
      roughness, density, strength, elongation, moisture, color, etc.

    Returns dict with process_cols, quality_cols, unknown_cols, and confidence.
    """
    process_kw = [
        'temp', 'temperature', 'press', 'speed', 'flow', 'rpm', 'torque',
        'volt', 'current', 'amp', 'power', 'freq', 'level', 'setpoint',
        'cmd', 'command', 'duty', 'pwm', 'pid', 'heater', 'cool', 'fan',
        'pump', 'valve', 'motor', 'feed',
        'melting_temp', 'extrusion', 'screw', 'barrel', 'chiller',
        'tension', 'draw_ratio', 'stretch', 'wind', 'unwind',
        'gap', 'nip', 'coater', 'dryer', 'oven', 'cure',
        'load', 'output', 'input', 'actuator', 'controller',
    ]
    quality_kw = [
        'thick', 'defect', 'weight', 'dim', 'diameter', 'grade', 'rough',
        'density', 'strength', 'elong', 'moisture', 'color', 'gloss',
        'haze', 'clarity', 'adhesion', 'scratch', 'bubble', 'gel',
        'contam', 'particle', 'pinhole', 'crack', 'warp', 'shrink',
        'yield', 'scrap', 'reject', 'quality', 'inspect', 'measure',
        'profile_deviation', 'cv', 'uniformity', 'cpk', 'sigma',
        'deviation', 'variance', 'stddev', 'range', 'coating_weight',
    ]
    ...
    for col in numeric_columns:
        ...
        is_process = any(kw in col_lower for kw in process_kw)
        is_quality = any(kw in col_lower for kw in quality_kw)

        if is_process and not is_quality:
            process_cols.append(col)
        elif is_quality and not is_process:
            quality_cols.append(col)
        elif is_process and is_quality:
            process_cols.append(col)
        else:
            unknown_cols.append(col)

    confidence = 'high' if (len(unknown_cols) <= len(numeric_columns) * 0.3) else 'medium'
    ...
    return {
        ...
        'note': f'Classified by keyword heuristics. {len(unknown_cols)}/{len(numeric_columns)} columns unrecognized.'
    }
...
# ═══════════════════════════════════════════════════════════════
#  TIME-MATCHED PROCESS ↔ QUALITY PRIMITIVES (v5.0)
#  ─────────────────────────────────────────────────────────────
#  Core insight: Process parameter at time T affects quality at
#  time T+ΔT (process lag). These primitives reveal the full
#  time-offset dependency structure between process and quality.
# ═══════════════════════════════════════════════════════════════

def compute_ccf_lag(x, y, max_lag=None):
    """
    Compute cross-correlation at each lag. Pure numpy — no scipy needed.

    Parameters
    ----------
    x, y : 1D arrays (same length, aligned by row index)
    max_lag : int or None (default: len(x)//4, capped at 50)

    Returns
    -------
    dict with best_lag, best_r, ccf_data (list of {lag, r})
    """
    ...
    for lag in range(-max_lag, max_lag + 1):
        if lag < 0:
            r = np.corrcoef(x_std[-lag:], y_std[:lag])[0, 1] if abs(lag) < n else 0
        elif lag > 0:
            r = np.corrcoef(x_std[:-lag], y_std[lag:])[0, 1] if lag < n else 0
        else:
        ...
        ccf_data.append({'lag': int(lag), 'r': float(r)})
        if abs(r) > abs(best_r):
            best_r = r
            best_lag = int(lag)
...
def plot_process_quality_dual_timeline(fig, ax_process, ax_quality, time,
                                        process_signals, quality_signals,
                                        lag_pairs=None, anomalies=None):
    """
    Dual-panel: process parameters (top) + quality metrics (bottom).
    Shared X-axis. CCF-derived lag arrows connect process changes to quality responses.

    This is the PRIMARY time-matching visualization for industrial diagnostics.

    Parameters
    ----------
    fig : matplotlib Figure
    ax_process : Axes for process signals (top)
    ax_quality : Axes for quality signals (bottom)
    time : array-like (datetime)
    process_signals : {label: array-like}
    quality_signals : {label: array-like}
    lag_pairs : list of {process_signal, quality_signal, best_lag, best_r}
    ...
    dict — generation metadata for manifest
    """
    colors_process = plt.cm.Reds(np.linspace(0.3, 0.9, max(len(process_signals), 1)))
    for idx, (label, values) in enumerate(process_signals.items()):
        ax_process.plot(time, values, linewidth=0.8, label=label,
                       color=colors_process[idx % len(colors_process)])
    ax_process.set_ylabel('Process Params', fontsize=9)
    ax_process.legend(loc='upper left', fontsize=6, ncol=2)
    ax_process.grid(True, alpha=0.3)
    _highlight_anomalies(ax_process, anomalies)
    ax_process.set_title('Process Parameters (Cause)', fontsize=10, fontweight='bold',
                         color='#c0392b')

    colors_quality = plt.cm.Blues(np.linspace(0.3, 0.9, max(len(quality_signals), 1)))
    for idx, (label, values) in enumerate(quality_signals.items()):
        ax_quality.plot(time, values, linewidth=1.0, label=label,
                       color=colors_quality[idx % len(colors_quality)])
    ax_quality.set_ylabel('Quality Metrics', fontsize=9)
    ax_quality.legend(loc='upper left', fontsize=6, ncol=2)
    ax_quality.grid(True, alpha=0.3)
    _highlight_anomalies(ax_quality, anomalies)
    ax_quality.set_title('Quality / Inspection (Effect)', fontsize=10, fontweight='bold',
                        color='#2980b9')

    ax_quality.xaxis.set_major_formatter(mdates.DateFormatter('%m-%d %H:%M'))
    fig.autofmt_xdate()

    lag_info = []
    if lag_pairs:
        for pair in lag_pairs[:5]:
            proc_sig = pair['process_signal']
            qual_sig = pair['quality_signal']
            best_lag = pair['best_lag']
            best_r = pair['best_r']
            ...
            mid_idx = len(time) // 2
            t_proc = pd.to_datetime(time.iloc[mid_idx])
            if len(time) >= 2:
                dt_per_row = (pd.to_datetime(time.iloc[-1]) - pd.to_datetime(time.iloc[0])) / len(time)
                t_qual = t_proc + dt_per_row * best_lag
            else:
                t_qual = t_proc

            color = '#e74c3c' if abs(best_r) > 0.5 else '#f39c12'
            ax_process.annotate('',
                xy=(t_qual, ax_quality.get_ylim()[0]),
                xytext=(t_proc, ax_process.get_ylim()[1]),
    ...
    fig.suptitle('Process -> Quality Time-Matched Analysis', fontweight='bold', fontsize=12)

    return {
        "function": "plot_process_quality_dual_timeline",
        "n_process_signals": len(process_signals),
        "n_quality_signals": len(quality_signals),
        "lag_pairs": lag_info,
        "anomaly_highlighting": bool(anomalies),
    }


def plot_lag_shifted_overlay(ax, time, quality_signal, quality_label,
                              process_signals_lag_shifted, best_lags):
    """
    Shift process signals by their optimal CCF lag to align with quality signal.
    Reveals hidden coupling: when shifted, process should track quality variations.

    Parameters
    ----------
    ax : Axes
    time : array-like (datetime)
    quality_signal : 1D array — reference quality metric
    quality_label : str
    process_signals_lag_shifted : {label: shifted_array}
    best_lags : {label: lag_value}

    Returns
    -------
    dict — generation metadata
    """
    qual_norm = normalize_01(quality_signal)
    ax.plot(time, qual_norm, linewidth=2.0, color='black', label=f'{quality_label} (quality ref)',
            zorder=10)

    colors = plt.cm.tab10(np.linspace(0, 1, max(len(process_signals_lag_shifted), 1)))
    for idx, (label, shifted) in enumerate(process_signals_lag_shifted.items()):
        proc_norm = normalize_01(shifted)
        lag = best_lags.get(label, 0)
        ax.plot(time, proc_norm, linewidth=0.8, color=colors[idx % len(colors)],
                alpha=0.7, label=f'{label} (lag={lag})')

    ax.set_ylabel('Normalized [0, 1]', fontsize=9)
    ax.set_ylim(-0.05, 1.05)
    ax.legend(loc='upper left', bbox_to_anchor=(1.01, 1), fontsize=7)
    ax.grid(True, alpha=0.3)
    ax.set_title(f'Lag-Shifted Overlay vs {quality_label}', fontsize=10, fontweight='bold')

    return {
        "function": "plot_lag_shifted_overlay",
        "quality_reference": quality_label,
        "n_process_signals": len(process_signals_lag_shifted),
        "best_lags": {k: int(v) for k, v in best_lags.items()},
    }


def plot_process_quality_lag_scatter(ax, process_values, quality_values,
                                      proc_label, qual_label, best_lag, best_r,
                                      time=None):
    """
    Scatter: quality at T+lag vs process at T.
    Shows predictive relationship with explicit time-offset annotation.

    Parameters
    ----------
    ax : Axes
    process_values, quality_values : 1D arrays (original, unshifted)
    proc_label, qual_label : str
    best_lag : int — positive = process LEADS quality
    best_r : float
    time : array-like or None — color points by time progression

    Returns
    -------
    dict — generation metadata
    """
    p = np.array(process_values, dtype=float)
    q = np.array(quality_values, dtype=float)

    if best_lag > 0:
        p_aligned = p[:-best_lag] if best_lag < len(p) else p
        q_aligned = q[best_lag:] if best_lag < len(q) else q
    elif best_lag < 0:
        p_aligned = p[-best_lag:] if abs(best_lag) < len(p) else p
        q_aligned = q[:best_lag] if abs(best_lag) < len(q) else q
    else:
        p_aligned = p
        q_aligned = q

    min_len = min(len(p_aligned), len(q_aligned))
    p_aligned = p_aligned[:min_len]
    q_aligned = q_aligned[:min_len]

    mask = ~(np.isnan(p_aligned) | np.isnan(q_aligned))
    p_aligned = p_aligned[mask]
    q_aligned = q_aligned[mask]

    if time is not None and hasattr(time, '__len__') and len(time) > 0:
        t_vals = np.arange(len(p_aligned))
        sc = ax.scatter(p_aligned, q_aligned, c=t_vals, cmap='viridis', s=4, alpha=0.5)
        plt.colorbar(sc, ax=ax, label='Time progression')
    else:
        ax.scatter(p_aligned, q_aligned, s=4, alpha=0.5)

    if len(p_aligned) >= 3:
        coeffs = np.polyfit(p_aligned, q_aligned, 1)
        x_line = np.linspace(p_aligned.min(), p_aligned.max(), 100)
        ax.plot(x_line, np.polyval(coeffs, x_line), 'r--', linewidth=1.5, alpha=0.7)

    ax.set_xlabel(f'{proc_label} (at time T)', fontsize=9)
    ax.set_ylabel(f'{qual_label} (at time T+{best_lag})', fontsize=9)
    ax.set_title(f'Process->Quality Lag Scatter\n'
                f'{proc_label} -> {qual_label}  |  lag={best_lag}, r={best_r:.3f}',
                fontsize=10)
    ax.grid(True, alpha=0.3)

    return {
        "function": "plot_process_quality_lag_scatter",
        "process_signal": proc_label,
        "quality_signal": qual_label,
        "best_lag": int(best_lag),
        "best_r": float(best_r),
        "n_points": len(p_aligned),
        "lag_interpretation": f"Process '{proc_label}' at T predicts '{qual_label}' at T+{best_lag}"
    }


def plot_multi_lag_correlation_heatmap(ax, ccf_results, title='Process x Lag Correlation'):
    """
    Heatmap: process-quality pairs (rows) x lag offsets (columns).
    Color = Pearson r. Star marker = best lag per pair.
    Reveals full time-offset structure across all process-quality relationships.

    Parameters
    ----------
    ax : Axes
    ccf_results : {pair_label: {ccf_data: [{lag, r}], best_lag, best_r}}
    title : str
    ...
    dict — generation metadata
    """
    pair_labels = list(ccf_results.keys())
    if not pair_labels:
        return {"function": "plot_multi_lag_correlation_heatmap", "skipped": True, "reason": "no pairs"}

    all_lags = set()
    for result in ccf_results.values():
        for d in result.get('ccf_data', []):
            all_lags.add(d['lag'])
    lags = sorted(all_lags)

    if not lags:
        return {"function": "plot_multi_lag_correlation_heatmap", "skipped": True, "reason": "no lag data"}

    matrix = np.zeros((len(pair_labels), len(lags)))
    best_lags_info = []

    for i, label in enumerate(pair_labels):
        result = ccf_results[label]
        ccf_dict = {d['lag']: d['r'] for d in result.get('ccf_data', [])}
        for j, lag in enumerate(lags):
            matrix[i, j] = ccf_dict.get(lag, 0)
        best_lags_info.append({
            'pair': label,
            'best_lag': result.get('best_lag', 0),
            'best_r': result.get('best_r', 0),
        })
    ...
    for i, info in enumerate(best_lags_info):
        best_lag = info['best_lag']
        if best_lag in lags:
            j = lags.index(best_lag)
            ax.plot(j, i, marker='*', color='black', markersize=12, markeredgecolor='white',
                   markeredgewidth=0.5)

    ax.set_xticks(range(len(lags)))
    ax.set_xticklabels([str(l) for l in lags], rotation=45, ha='right', fontsize=7)
    ax.set_yticks(range(len(pair_labels)))
    ax.set_yticklabels(pair_labels, fontsize=7)
    ax.set_xlabel('Time Lag (rows)', fontsize=9)
    ax.set_ylabel('Process -> Quality Pair', fontsize=9)
    ax.set_title(title, fontsize=10, fontweight='bold')
    plt.colorbar(im, ax=ax, label='Pearson r', shrink=0.8)

    return {
        "function": "plot_multi_lag_correlation_heatmap",
        "n_pairs": len(pair_labels),
        "n_lags": len(lags),
        "best_lags": {info['pair']: {'lag': info['best_lag'], 'r': info['best_r']}
                      for info in best_lags_info},
    }


def plot_rolling_window_correlation(ax, time, process_signal, quality_signal,
                                     proc_label, qual_label, window_size=None,
                                     best_lag=0, anomalies=None):
    """
    Rolling-window Pearson r between process and quality over time.
    Reveals WHEN the process-quality relationship strengthens or breaks down.

    Parameters
    ----------
    ax : Axes
    time : array-like (datetime)
    process_signal, quality_signal : 1D arrays
    proc_label, qual_label : str
    window_size : int or None (default: len(data)//10, min 10)
    best_lag : int — apply this lag shift before computing rolling r
    anomalies : list or None
    ...
    dict — generation metadata
    """
    p = np.array(process_signal, dtype=float)
    q = np.array(quality_signal, dtype=float)

    if best_lag > 0:
        p_align = p[:-best_lag] if best_lag < len(p) else p
        q_align = q[best_lag:] if best_lag < len(q) else q
    elif best_lag < 0:
        p_align = p[-best_lag:] if abs(best_lag) < len(p) else p
        q_align = q[:best_lag] if abs(best_lag) < len(q) else q
    else:
        p_align = p
        q_align = q

    min_len = min(len(p_align), len(q_align))
    p_align = p_align[:min_len]
    q_align = q_align[:min_len]
    t_align = time.iloc[:min_len] if hasattr(time, 'iloc') else time[:min_len]

    if window_size is None:
        window_size = max(min_len // 10, 10)

    rolling_r = []
    rolling_t = []
    half = window_size // 2

    for i in range(half, min_len - half):
        p_win = p_align[i - half : i + half]
        q_win = q_align[i - half : i + half]
        mask = ~(np.isnan(p_win) | np.isnan(q_win))
        if mask.sum() >= 5:
            r = np.corrcoef(p_win[mask], q_win[mask])[0, 1]
            if not np.isnan(r):
                rolling_r.append(r)
                rolling_t.append(t_align.iloc[i] if hasattr(t_align, 'iloc') else t_align[i])

    if not rolling_r:
        return {"function": "plot_rolling_window_correlation", "skipped": True, "reason": "insufficient data"}

    ax.fill_between(rolling_t, 0, rolling_r, alpha=0.3, color='#3498db')
    ax.plot(rolling_t, rolling_r, linewidth=1.2, color='#2980b9')
    ax.axhline(y=0, color='black', linewidth=0.5)
    ax.axhline(y=np.mean(rolling_r), color='green', linestyle='--', linewidth=1.0,
              alpha=0.7, label=f'Mean r={np.mean(rolling_r):.3f}')
    ax.axhline(y=0.7, color='red', linestyle=':', alpha=0.5, label='|r|=0.7')
    ax.axhline(y=-0.7, color='red', linestyle=':', alpha=0.5)

    _highlight_anomalies(ax, anomalies)
    ax.set_ylabel(f'Rolling r (win={window_size})', fontsize=9)
    ax.set_title(f'Rolling Correlation: {proc_label} -> {qual_label} (lag={best_lag})',
                fontsize=10)
    ax.legend(fontsize=7)
    ax.grid(True, alpha=0.3)

    return {
        "function": "plot_rolling_window_correlation",
        "process_signal": proc_label,
        "quality_signal": qual_label,
        "window_size": window_size,
        "best_lag": int(best_lag),
        "mean_r": float(np.mean(rolling_r)),
        "std_r": float(np.std(rolling_r)) if len(rolling_r) > 1 else 0,
        "n_windows": len(rolling_r),
    }


def plot_lag_sensitivity_curve(ax, ccf_data, best_lag, best_r, proc_label, qual_label):
    """
    Full CCF curve: correlation vs lag for one process-quality pair.
    Reveals the complete time-offset dependency structure.

    Interpretation:
    - Peak lag = strongest causal delay (process->quality propagation time)
    - Curve width = persistence of relationship across lags (wide = robust)
    - Asymmetry = symmetric CCF suggests bidirectional or common-driver;
      asymmetric CCF suggests directional causal relationship

    Parameters
    ----------
    ax : Axes
    ccf_data : list of {lag, r}
    best_lag, best_r : float
    proc_label, qual_label : str
    ...
    dict — generation metadata
    """
    lags = [d['lag'] for d in ccf_data]
    rs = [d['r'] for d in ccf_data]
    ...
    ax.bar(lags, rs, color=colors, alpha=0.85, width=0.8)
    ax.axhline(y=0, color='black', linewidth=0.8)
    ax.axvline(x=0, color='gray', linestyle='--', alpha=0.5, label='Lag=0 (concurrent)')
    ax.axvline(x=best_lag, color='red', linestyle='-', linewidth=2, alpha=0.8,
              label=f'Best lag={best_lag}, r={best_r:.3f}')

    if best_lag in lags:
        ax.plot(best_lag, best_r, 'r*', markersize=15, markeredgecolor='darkred')

    if best_lag > 0:
        direction = f'{proc_label} LEADS {qual_label} by {best_lag} rows'
    elif best_lag < 0:
        direction = f'{qual_label} LEADS {proc_label} by {abs(best_lag)} rows'
    else:
        direction = 'Signals are concurrent (no lag)'

    pos_lags = [d['r'] for d in ccf_data if d['lag'] > 0]
    neg_lags = [d['r'] for d in ccf_data if d['lag'] < 0]
    asym_note = ''
    if pos_lags and neg_lags:
        max_pos = max(abs(r) for r in pos_lags)
        max_neg = max(abs(r) for r in neg_lags)
        asym_ratio = (max_pos - max_neg) / (max(max_pos, max_neg, 0.01))
        if abs(asym_ratio) > 0.3:
            asym_note = 'Asymmetric CCF -> directional causal relationship'
        else:
            asym_note = 'Symmetric CCF -> bidirectional or common-driver'

    ax.set_xlabel('Lag (rows)', fontsize=10)
    ax.set_ylabel('Pearson r', fontsize=10)
    ax.set_title(f'Lag Sensitivity: {proc_label} -> {qual_label}\n{direction}',
                fontsize=10)
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3, axis='y')

    return {
        "function": "plot_lag_sensitivity_curve",
        "process_signal": proc_label,
        "quality_signal": qual_label,
        "best_lag": int(best_lag),
        "best_r": float(best_r),
        "n_lags": len(lags),
        "ccf_asymmetry": asym_note,
...
def main():
    ...
    # ═══════════════════════════════════════════════════════════
    # TIME-MATCHED PROCESS ↔ QUALITY ANALYSIS (v5.0)
    # Core: Process param at T → Quality metric at T+ΔT
    # ═══════════════════════════════════════════════════════════
    process_cols = pattern.get('process_columns', [])
    quality_cols = pattern.get('quality_columns', [])

    if pattern.get('pq_classification_confidence') == 'medium':
        process_cols = process_cols + pattern.get('unknown_columns', [])

    has_pq_data = (len(process_cols) >= 1 and len(quality_cols) >= 1
                   and TIME_COL and TIME_COL in df.columns)

    if has_pq_data:
        print(f"\nTime-Matched P->Q: {len(process_cols)} process, {len(quality_cols)} quality")

        ccf_results = {}
        lag_pairs_for_timeline = []

        for proc_col in process_cols:
            for qual_col in quality_cols:
                if proc_col in df.columns and qual_col in df.columns:
                    pair_label = f'{proc_col}->{qual_col}'
                    result = compute_ccf_lag(df[proc_col].values, df[qual_col].values)
                    ccf_results[pair_label] = result

                    if abs(result['best_r']) > 0.3:
                        lag_pairs_for_timeline.append({
                            'process_signal': proc_col,
                            'quality_signal': qual_col,
                            'best_lag': result['best_lag'],
                            'best_r': result['best_r'],
                        })

        lag_pairs_for_timeline.sort(key=lambda x: abs(x['best_r']), reverse=True)

        if ccf_results:
            # 5a. Process-Quality Dual Timeline
            if len(process_cols) >= 1 and len(quality_cols) >= 1:
                proc_sigs = {c: df[c].values for c in process_cols[:6] if c in df.columns}
                qual_sigs = {c: df[c].values for c in quality_cols[:6] if c in df.columns}

                fig, (ax_p, ax_q) = plt.subplots(2, 1, figsize=(20, 10), sharex=True)
                meta = plot_process_quality_dual_timeline(
                    fig, ax_p, ax_q, df[TIME_COL],
                    proc_sigs, qual_sigs,
                    lag_pairs=lag_pairs_for_timeline,
                    anomalies=ANOMALY_INTERVALS
                )
                fig.tight_layout()
                fig.savefig(out / "05a_process_quality_timeline.png", dpi=DPI,
                           bbox_inches='tight', facecolor='white')
                plt.close(fig)
                plot_records.append({
                    "filename": "05a_process_quality_timeline.png",
                    "plot_type": "process_quality_dual_timeline",
                    "description": "Process params (top) vs quality metrics (bottom) with CCF lag arrows",
                    "generation_method": meta,
                    "key_features": "Lag arrows show process->quality propagation delay",
                    "anomaly_highlighted": bool(ANOMALY_INTERVALS),
                })
                print("  -> 05a_process_quality_timeline.png")

            # 5b. Multi-Lag Correlation Heatmap
            if len(ccf_results) >= 2:
                fig, ax = plt.subplots(figsize=(max(10, len(ccf_results)*1.2),
                                                max(6, len(ccf_results)*0.5)))
                meta = plot_multi_lag_correlation_heatmap(ax, ccf_results)
                fig.tight_layout()
                fig.savefig(out / "05b_multi_lag_heatmap.png", dpi=DPI,
                           bbox_inches='tight', facecolor='white')
                plt.close(fig)
                plot_records.append({
                    "filename": "05b_multi_lag_heatmap.png",
                    "plot_type": "multi_lag_correlation_heatmap",
                    "description": "Process x Lag correlation heatmap — full time-offset structure",
                    "generation_method": meta,
                    "key_features": "Star=best lag. Off-diagonal hotspots=delayed causal links.",
                    "anomaly_highlighted": False,
                })
                print("  -> 05b_multi_lag_heatmap.png")

            # 5c. Lag-Shifted Overlay
            if quality_cols and process_cols:
                top_qual = quality_cols[0]
                top_pairs = sorted(
                    [(pc, ccf_results.get(f'{pc}->{top_qual}', {}).get('best_r', 0),
                      ccf_results.get(f'{pc}->{top_qual}', {}).get('best_lag', 0))
                     for pc in process_cols[:8]],
                    key=lambda x: abs(x[1]), reverse=True
                )[:4]

                shifted_signals = {}
                best_lags = {}
                for proc_col, best_r, best_lag in top_pairs:
                    if abs(best_r) < 0.2:
                        continue
                    p = df[proc_col].values
                    if best_lag > 0:
                        shifted = np.roll(p, -best_lag)
                        shifted[-best_lag:] = np.nan
                    elif best_lag < 0:
                        shifted = np.roll(p, -best_lag)
                        shifted[:-best_lag] = np.nan
                    else:
                        shifted = p
                    shifted_signals[proc_col] = shifted
                    best_lags[proc_col] = best_lag

                if shifted_signals:
                    fig, ax = plt.subplots(figsize=(16, 6))
                    meta = plot_lag_shifted_overlay(ax, df[TIME_COL], df[top_qual].values,
                                                    top_qual, shifted_signals, best_lags)
                    fig.tight_layout()
                    fig.savefig(out / "05c_lag_shifted_overlay.png", dpi=DPI,
                               bbox_inches='tight', facecolor='white')
                    plt.close(fig)
                    plot_records.append({
                        "filename": "05c_lag_shifted_overlay.png",
                        "plot_type": "lag_shifted_overlay",
                        "description": f"Process signals shifted by optimal lag to align with {top_qual}",
                        "generation_method": meta,
                        "key_features": "Aligned process tracks quality — confirms causal delay",
                        "anomaly_highlighted": False,
                    })
                    print("  -> 05c_lag_shifted_overlay.png")

            # 5d. Top Process-Quality Lag Scatter
            if lag_pairs_for_timeline:
                top_pair = lag_pairs_for_timeline[0]
                proc_col = top_pair['process_signal']
                qual_col = top_pair['quality_signal']

                fig, ax = plt.subplots(figsize=(8, 8))
                meta = plot_process_quality_lag_scatter(
                    ax, df[proc_col].values, df[qual_col].values,
                    proc_col, qual_col, top_pair['best_lag'], top_pair['best_r'],
                    time=True
                )
                fig.tight_layout()
                fig.savefig(out / "05d_lag_scatter.png", dpi=DPI,
                           bbox_inches='tight', facecolor='white')
                plt.close(fig)
                plot_records.append({
                    "filename": "05d_lag_scatter.png",
                    "plot_type": "process_quality_lag_scatter",
                    "description": f"{proc_col} at T -> {qual_col} at T+{top_pair['best_lag']} (r={top_pair['best_r']:.3f})",
                    "generation_method": meta,
                    "key_features": "Scatter confirms if lag-shifted process linearly predicts quality",
                    "anomaly_highlighted": False,
                })
                print("  -> 05d_lag_scatter.png")

            # 5e. Lag Sensitivity Curves (top 3 pairs)
            top_3_pairs = lag_pairs_for_timeline[:3]
            if top_3_pairs:
                n_curves = len(top_3_pairs)
                fig, axes = plt.subplots(n_curves, 1, figsize=(14, 4 * n_curves), squeeze=False)
                curve_info = []
                for idx, pair in enumerate(top_3_pairs):
                    proc_col = pair['process_signal']
                    qual_col = pair['quality_signal']
                    pair_label = f'{proc_col}->{qual_col}'
                    result = ccf_results.get(pair_label, {})
                    ccf_data = result.get('ccf_data', [])

                    if ccf_data:
                        plot_lag_sensitivity_curve(
                            axes[idx, 0], ccf_data,
                            pair['best_lag'], pair['best_r'],
                            proc_col, qual_col
                        )
                        curve_info.append({
                            'process': proc_col,
                ...
                fig.suptitle('Lag Sensitivity Analysis — Process->Quality CCF Curves',
                           fontweight='bold', fontsize=12)
                fig.tight_layout()
                fig.savefig(out / "05e_lag_sensitivity_curves.png", dpi=DPI,
                           bbox_inches='tight', facecolor='white')
                plt.close(fig)

                plot_records.append({
                    "filename": "05e_lag_sensitivity_curves.png",
                    "plot_type": "lag_sensitivity_curve",
                    "description": "Full CCF curves for top process-quality pairs",
                    "generation_method": {
                        "function": "plot_lag_sensitivity_curve",
                        "curves": curve_info,
                    },
                    "key_features": "Peak lag=causal delay. Curve width=persistence. Asymmetry=directionality.",
                    "anomaly_highlighted": False,
                })
                print("  -> 05e_lag_sensitivity_curves.png")

            # 5f. Rolling Window Correlation (top pair)
            if lag_pairs_for_timeline and TIME_COL:
                top_pair = lag_pairs_for_timeline[0]
                proc_col = top_pair['process_signal']
                qual_col = top_pair['quality_signal']

                fig, ax = plt.subplots(figsize=(16, 5))
                meta = plot_rolling_window_correlation(
                    ax, df[TIME_COL], df[proc_col].values, df[qual_col].values,
                    proc_col, qual_col,
                    best_lag=top_pair['best_lag'],
                    anomalies=ANOMALY_INTERVALS
                )

                if not meta.get('skipped'):
                    fig.tight_layout()
                    fig.savefig(out / "05f_rolling_correlation.png", dpi=DPI,
                               bbox_inches='tight', facecolor='white')
                    plot_records.append({
                        "filename": "05f_rolling_correlation.png",
                        "plot_type": "rolling_window_correlation",
                        "description": f"Rolling window r: {proc_col} -> {qual_col} over time",
                        "generation_method": meta,
                        "key_features": "Shows WHEN the process->quality relationship strengthens/weakens",
                        "anomaly_highlighted": bool(ANOMALY_INTERVALS),
                    })
                    print("  -> 05f_rolling_correlation.png")
                plt.close(fig)
    else:
        if TIME_COL:
            print(f"\n[skip] Time-Matched P->Q: need 1+ process & 1+ quality columns "
                  f"(got {len(process_cols)} process, {len(quality_cols)} quality)")
