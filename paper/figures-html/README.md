...
> ⚠️ **DEPRECATED (2026-09-15)**: the journal submission no longer uses HTML-screenshot
> figures. All result figures are now vector PDFs generated by `../figures/make_figures.py`
> (matplotlib, data-driven from `results/benchmark/`), and the two architecture/flow
> diagrams are native TikZ in `../main.tex`. This folder is retained only as the
> design-phase provenance of the visual grammar (palette, layout ideas).
