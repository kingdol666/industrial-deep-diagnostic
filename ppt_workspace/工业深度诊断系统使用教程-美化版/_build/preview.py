...
OUT = os.path.join(HERE, "_build", "_preview")
...
def _extract_array(text, start):
    """Return the substring of the [...] block beginning at `start`."""
    depth, i = 0, start
    while i < len(text):
        if text[i] == "[":
            depth += 1
...
def _brace_block(text, key):
    """The {...} block that follows `key:` — so colour lookups stay scoped
    (an unscoped search would pick up cellStyle's background colour and paint
    the text in it)."""
    m = re.search(re.escape(key) + r":\s*\{", text)
    if not m:
        return ""
    return _extract_array_braces(text, m.end() - 1)


def _extract_array_braces(text, start):
    depth, i = 0, start
    while i < len(text):
        if text[i] == "{":
            depth += 1
...
def _cell_parts(cell):
    """({text, bold, color, bg}) for one Table cell literal."""
    text, bold, color, bg = "", False, None, None
    if cell.startswith("{"):
        m = re.search(r"text:\s*(['\"])(.*?)\1", cell, re.S)
        if m:
            text = m.group(2)
        ts = _brace_block(cell, "textStyle")
        if re.search(r"bold:\s*true", ts):
            bold = True
        m = re.search(r"color:\s*(['\"])(#[0-9A-Fa-f]{3,8})\1", ts)
        if m:
            color = m.group(2)
        cs = _brace_block(cell, "cellStyle")
        m = re.search(r"color:\s*(['\"])([^'\"]+)\1", cs)
        if m:
            bg = m.group(2)
    else:
        m = re.fullmatch(r"(['\"])(.*?)\1", cell, re.S)
        text = m.group(2) if m else cell
    return text, bold, color, bg


def table_html(tag_attrs):
    """Render a SlideDSL <Table> as a plain HTML table."""
    m = re.search(r"cells=\{\s*\[", tag_attrs)
    if not m:
        return "<table></table>"
    inner = _extract_array(tag_attrs, m.end() - 1)[1:-1]
    rows = []
    for row in _split_top(inner):
        if not (row.startswith("[") and row.endswith("]")):
            continue
        cells = _split_top(row[1:-1])
        tds = []
        for c in cells:
            text, bold, color, bg = _cell_parts(c)
            st = "padding:9px 14px;border:1px solid #E4DCCB;vertical-align:middle;"
            if bold:
                st += "font-weight:700;"
            st += f"color:{color};" if color else "color:#1D1B16;"
            st += f"background:{bg};" if bg else ""
            tds.append(f'<td style="{st}">{text}</td>')
        rows.append("<tr>" + "".join(tds) + "</tr>")
    m2 = re.search(r"defaultTextStyle=\{\{(.*?)\}\}", tag_attrs, re.S)
    fs = "17"
    if m2:
        m3 = re.search(r"fontSize:\s*(\d+)", m2.group(1))
        if m3:
            fs = m3.group(1)
    return (f'<table style="width:100%;height:100%;border-collapse:collapse;'
            f'font-size:{fs}px;table-layout:fixed">' + "".join(rows) + "</table>")
...
def convert(src: str) -> str:
    ...
    # Table props contain their own braces/quotes; hand the whole element aside
    tables: list[str] = []

    def _stash_table(m):
        tables.append(m.group(0))
        return f"<TableRef n=\"{len(tables) - 1}\"></TableRef>"

    src = re.sub(r"<Table\b.*?(?:/>|</Table>)", _stash_table, src, flags=re.S)
    ...
    for m in TAG_RE.finditer(src):
        ...
        if tag == "TableRef":
            n = re.search(r'n="(\d+)"', attrs or "")
            out.append(table_html(tables[int(n.group(1))]) if n else "<table></table>")
        elif tag == "Image":
...
def main() -> int:
    ...
    html_dir = os.path.join(HERE, "_build", "_preview-html")
