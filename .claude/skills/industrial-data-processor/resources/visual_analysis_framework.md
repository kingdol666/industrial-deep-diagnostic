...
### 关键图表类型：时间对齐叠加图（有时间列时优先生成）

这是 VLM 最能提取诊断信息的图表类型，也是 **Data Processor 在存在有效时间列时应优先生成的主图**。当数据具备可用时间列时，应把关键质量指标和关键过程参数在**同一个图中**按照**相同 x 时间轴**对齐绘制。将所有参数归一化后叠加在同一个时间轴上：
...
1. **一个图里放多个关键参数**，不能把每个参数拆成完全独立、互不对齐的图。
2. **所有曲线使用相同 x 轴**，并且该要求建立在存在有效时间列的前提上；优先真实时间，不建议在没有时间列时机械伪造“时序对齐”。
3. **至少包含**：
   - 主要质量目标
   - Top 候选原因参数
   - 关键事件标记
   - 异常区间或退化区间标识（如果有）
4. **归一化后再叠加**，避免量纲差异掩盖时序关系。
5. 如果系列太多，允许只保留最关键的 8-12 条线，但必须保证“质量目标 + 候选原因 + 事件”同时出现在同一张主图中。
6. 这张图应作为后续视觉分析的**第一读图对象**。
7. 如果数据**没有有效时间列**，不要机械生成伪时间对齐图。应明确声明“时间对齐不适用”，并改用最强的非时序图表（如分层散点、分组箱线、分布对比、事件前后对比图等）。
...
**读取顺序要求：**
1. 如果存在有效时间列，先读“主时间对齐叠加图”
2. 从主图中先提取同步组、领先/滞后线索、事件响应窗口
3. 再读散点图、事件响应图、Simpson 图、热图等，用于确认或反驳主图里的视觉判断
4. 如果不存在有效时间列，则从最强的非时序总览图开始读起
...
对于主时间对齐叠加图（若存在），`visual_analysis.json` 中必须至少包含：
- `synchronous_groups`
- `precedence_signals`
- `independent_parameters`
- 至少 3 条来自主图的 `visual_observations`
- 一段明确说明“为什么这张同轴对齐图支持/削弱哪些根因假设”的总结
...
```python
# 设计要点：
# 1. 所有关键参数归一化到同一尺度 (z-score)
# 2. 共享 x 轴（时间）——仅在存在有效时间列时适用
# 3. 多个质量指标与多个关键过程参数必须在同一个图中共同出现
# 4. 反转与质量负相关的参数（如温度）使所有参数在同一方向上变化
# 5. 事件标记为红色虚线 + 文字标注
# 6. 异常区间为红色阴影
# 7. 图例清晰，字体 ≥ 12pt
# 8. 纵轴标签: "归一化值 (z-score, 负相关参数已反转)"
