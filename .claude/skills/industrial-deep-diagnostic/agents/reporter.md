...
- **6.6 数据清洗留痕（必写）** — 明确告诉读者"结论基于哪份干净数据、清洗动过什么"。从 `data_analysis_conclusion.json.data_cleaning_provenance` 读取并表格化披露：
  - **清洗操作表**：每一行一项（去重 / 排序 / 类型修复 / 缺失处理 / 异常值 / 派生特征）→ 目标列/对象 → 影响行数 → 为什么这么做（rationale）
  - **数据源决策**：最终用的是 `cleaned` 还是 `raw_fallback`，原因是什么（如类型泄漏无法修复→回退 raw）
  - **完整性对账**：原始行数 vs 清洗后行数、丢弃率；类型修复了哪些列的 stray tokens（如 `<0.05`/`N/A`）
  - 若某项清洗未执行，也要写"未执行 + 原因"（如"未做异常值剔除——异常点本身就是诊断目标"），**不得省略**。这是让读者信任后续统计与结论的留痕基础
