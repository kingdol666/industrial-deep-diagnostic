...
| VLM visual analysis（Step 3.5） | **No** | figures + `ontology.json` + validation artifacts |
...
**关键约束**：`plot_manifest.json` 必须含至少一张通过上述 1-3 项的真图（有有效时间列时还应含至少一张时序对齐图）。否则后续 VLM 无图可读。
...
## Phase 5.x: VLM 视觉分析（已移至 Step 3.5）

VLM 视觉图像分析已从 data-processor 内部**分离为独立管线步骤 Step 3.5**。data-processor **不再负责**启动 vlm-visual-analyzer。

你作为 data-processor 的任务在 Phase 5 画图完成后即结束。vlm-visual-analyzer 将由主 Agent 在 Step 3.5 独立启动，并直接输出至 diagnostician。

**请勿在 data-processor 内部启动或引用 vlm-visual-analyzer。**
...
03_figures/visual_analysis.json               ← VLM visual image analysis (Step 3.5)
...
12. **VLM visual analysis is MANDATORY (Step 3.5).** After generating all charts, the main agent will launch vlm-visual-analyzer in Step 3.5. Charts are not decorative evidence — they are diagnostic input that a VLM Agent will actively read and reason from.
