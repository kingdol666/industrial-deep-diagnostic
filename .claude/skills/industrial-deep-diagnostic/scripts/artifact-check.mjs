...
function validateDataProcessorExpertContract() {
  ...
  // Unified checks: validated_correlations + diagnostician_handoff + cleaning provenance
  const pairs = conclusion.validated_correlations?.pairs;
  if (!Array.isArray(pairs)) {
    issues.push('validated_correlations.pairs missing');
  ...
  if (!Array.isArray(handoff.priority_hypothesis_inputs) || handoff.priority_hypothesis_inputs.length === 0) {
    issues.push('diagnostician_handoff.priority_hypothesis_inputs empty');
  }
  ...
  if (!provenance.cleaning_operations || !Array.isArray(provenance.cleaning_operations)) {
    issues.push('data_cleaning_provenance.cleaning_operations missing');
  }
  ...
  // V1 coverage matrix not used in unified schema; evidence artifacts come from selected_analyses above
  ...
  // Custom scripts check (expert_gap_analysis.custom_scripts_run)
  const customScriptsDeclared = conclusion.expert_gap_analysis?.custom_scripts_run || [];
...
const schemaChecks = [
  ...
  validate('Data Analysis Conclusion V2', 'schemas/data_analysis_conclusion_schema.json', '02_processed/data_analysis_conclusion.json'),
