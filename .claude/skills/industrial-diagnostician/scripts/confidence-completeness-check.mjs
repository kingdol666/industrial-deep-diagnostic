...
for (const h of breakdowns) {
  ...
  const missing = REQUIRED_FACTORS.filter(f => factors[f.key] === undefined);
  const outOfRange = REQUIRED_FACTORS.filter(f => {
    const v = factors[f.key];
    return typeof v === 'number' && (v < 0 || v > f.max);
  });

  const sum = REQUIRED_FACTORS.reduce((s, f) => s + (factors[f.key] || 0), 0);
