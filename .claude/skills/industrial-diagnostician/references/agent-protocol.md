...
### STEP A2: Chain Link Validation — 因果链每环自检（新增 v2.0 — 防 J/W 比式断裂）

> **本次执行断裂经验**：纸机流浆箱诊断中构建了"压力↑→J/W比偏离→CD CV↑"因果链，但J/W比与所有质量指标的 Pearson r≈0(p>0.05)，中间环节完全无数据支持。此问题直到 Step 7 审计才被发现。**每环自检确保因果链的每个"箭头"都有数据证据，不让无证据的物理推理滑入诊断结论。**

#### 操作方式

在因果链构建后（STEP A 完成后），**在执行任何排名之前**，执行 Chain Link Validation：

对于每个假设的因果链，将其拆分为一连串 `[cause] → [mechanism] → [intermediate] → [mechanism] → [effect]` 箭头。每个箭头必须验证：
...
| 检查 | 问题 | 数据来源 | Pass 条件 |
|------|------|---------|----------|
| **数据列存在性** | 箭头两端是否有对应的数据列？ | `feature_summary.json` 的 column_names | 两端都能映射到数据列名 |
| **统计相关性** | 两端之间在数据中是否有显著的相关系数？ | `feature_summary.json` 相关矩阵 | 对应列对的 raw r 或 Pearson r 在 feature_summary 中有条目 |
| **方向一致性** | 相关方向是否与箭头方向一致？ | `feature_summary.json` r 的符号 | (cause↑→effect↑) → r>0; (cause↑→effect↓) → r<0 |
| **物理可行性** | 该中间环节的量级在物理上是否可能？ | Magnitude check L4 | 量级比值 ≤10× (PLAUSIBLE) 或 ≤100× (BORDERLINE) |

#### 自检结果分类

| 结果 | 含义 | 动作 |
|------|------|------|
| **ALL_LINKS_VALIDATED** | 所有箭头都有数据支持 | 继续，无额外扣分 |
| **DATA_LINK_MISSING** | 某箭头两端无对应数据列 | 标记 `[INFERENCE_GAP: no_data_column_for_intermediate]`，该假设自动降为 `COMPETING_SET`（非 DETERMINED） |
| **CORRELATION_ABSENT** | 箭头两端存在但相关系数 r≈0 (p≥0.05) | 标记 `[INFERENCE_GAP: link_X_zero_correlation]`，在报告中诚实披露该环节无数据支持，confidence 在该因子上扣 -10 至 -20，不得隐藏 |
| **DIRECTION_MISMATCH** | 相关方向与物理箭头相反 | 标记 `[INFERENCE_GAP: direction_reversal_at_link_X]`，该箭头可能受控制回路或混淆变量干扰，需调查替代机制 |
| **MAGNITUDE_IMPLAUSIBLE** | 量级比 >100× | 标记 `[PHYSICS_MISMATCH: link_X_magnitude_implausible]`，该机制被排除 |

#### 诊断的因果链修正规则（基于纸机诊断经验）

**如果发现 CORRELATION_ABSENT**（如纸机诊断中 J/W 比-质量 r≈0）：

```json
{
  "causal_chain_original": "压力↑ → [Bernoulli: v_jet↑] → J/W比偏离1.0 → [纤维取向变化] → CD CV↑",
  "chain_link_validation_results": {
    "links": [
      {"from": "pressure", "to": "J/W比", "r": 0.0164, "p": 0.35, "result": "CORRELATION_ABSENT"},
      {"from": "J/W比", "to": "CD_CV", "r": 0.0164, "p": 0.35, "result": "CORRELATION_ABSENT"}
    ],
    "overall_result": "CORRELATION_ABSENT"
  },
  "chain_correction": "压力↑ → [直接影响流浆箱内流动分布均匀性] → CD CV↑（剔除J/W比中介环节）",
  "correction_evidence": "pressure~CDCV raw r=0.87, detrended r=0.57, UNIVERSAL across grades",
  "inference_gap_acknowledged": true,
  "inference_gap_detail": "J/W比在DCS中被精确控制在1.02±0.03（所有三纸种均值完全相同），因此其不变化是控制行为而非物理无关。压力影响CD均匀性的实际机制是通过改变流浆箱内部湍流强度或稀释水混合效率，而非通过J/W比中介。",
  "confidence_impact": -3
}
```

**核心原则**：
- 发现 CORRELATION_ABSENT → **不得静默删除该环节**。必须：(1) 在 `diagnosis.json.causal_chain_validation` 中记录； (2) 在 `reasoning_chain.json` R4 中标注 INFERENCE_GAP； (3) 在证据链中修正或诚实披露该 gap
- 如果因果链中**超过一个箭头**无数据支持 → 该假设不能为 `DETERMINED`，必须降为 `COMPETING_SET`
- Chain link validation 结果必须写入 `evidence.json.validation_evidence[]` 和 `diagnosis.json.inference_gaps[]`
...
- [ ] **CHAIN LINK VALIDATION done?** (STEP A2 executed: each causal arrow checked for data column existence + correlation significance + direction consistency)
- [ ] If CHAIN LINK shows CORRELATION_ABSENT — is it documented in inference_gaps[] and has the causal chain been corrected?
