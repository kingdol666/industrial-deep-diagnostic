...
//                   identically (over 80-sample / stride-20 sliding windows for
//                   TEP) for the training corpus. Standardised with TRAINING
...
//                   model was trained on (the triple is not scale-free: sd and
...
//                   per-node feature draws in a fixed order. The model cache is
//                   keyed on the training SOURCE (and file), so one user's model
//                   is never served for another's data.
 ...
 * bootstrap draws would resample a handful of rows and the "forest" would be a
 * memorised handful of aggregates, so the module refuses instead.
 */
...
const DEFAULT_NO_TRAINING_REASON =
  'No labelled training data is available for this upload, so the random forest was NOT applied and no prediction is '
  + 'made: a classifier needs labelled examples of each class and inventing them would be fabrication. Re-upload the '
  + 'dataset together with a labelled training CSV (one row per sample, the same sensor column names as the data, plus '
  + 'a class/label column) to make this algorithm usable.';
...
async function runUserTrained(ctx, t0) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    const p = predictProbabilities(model.trees, x, model.K);
    let best = 0;
  ...
  return {
    ...
    detection: {
      ...
      model: 'random forest (majority vote, vote fraction as probability), trained on the user-supplied labelled file',
    },
    training: {
      rows: model.trainRows,
      classes: model.K,
    ...
    reasoning:
      ...
      `${model.trees.length} bootstrap trees, maxDepth ${MAX_DEPTH}, mtry ${model.mtry}, Gini splits, ${model.totalNodes} nodes total; ` +
      `in-bag training accuracy ${(model.trainAccuracy * 100).toFixed(2)}%. ` +
      ...
      `; the vote fractions below are the mean over those windows and ${hits[top.k]}/${segments.length} of them agree on the top class. ` +
      `Prediction: '${predicted}' with vote fraction ${top.p.toFixed(4)}` +
      (runnerUp ? `, runner-up '${model.labelNames[runnerUp.k]}' with ${runnerUp.p.toFixed(4)}` : '') +
      `. Ranked top-3: ${top3.map((s, i) => `${i + 1}. ${s}`).join(' | ')}. ` +
      `Vote fractions are quantised in steps of 1/${model.trees.length}, so a probability of 0.5 is not a calibrated confidence but a near-tie in the forest. ` +
