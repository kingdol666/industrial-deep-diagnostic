...
- [ ] **5.0a** **MANDATORY — Adaptive chart planning + time-axis alignment.** Run `adaptive_charts.py "$RUN_DIR"` BEFORE any fixed chart step: it resolves/aligns the time axis (writes `02_processed/time_alignment.json` — parsing format, dropped rows, monotonicity, interval), detects the data shape per column (2D coordinate field / multi-parameter series / single channel), renders the shape-appropriate charts (2D field → filled contour cloud map; multi-param → z-overlay on the aligned axis; single channel → line), and records every decision in `03_figures/adaptive_chart_plan.json`. Temporal-precedence claims are only allowed on charts whose axis is a parsed real time column (never an index-implied axis). Add 5.0a output charts to `vlm_input_manifest` priorities (contour/overlay = MANDATORY).
